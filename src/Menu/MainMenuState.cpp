...
void MainMenuState::btnUpdateClick(Action*)
{
	...
	{
		SDL_RWops *rwo_read = zipGetFileByName(relativeExeZipFileName, file_to_extract);
		if (!rwo_read) {
			Log(LOG_ERROR) << "Step 10: failed to unzip file.";
			return;
		}
		size_t size = 0;
		auto data = SDL_LoadFile_RW(rv, &size, SDL_TRUE);
		if (!data) {
			Log(LOG_ERROR) << "Step 10: failed to unzip file." << SDL_GetError(); // out of memory for a copy ?
			return;
		}
		SDL_RWops *rwo_write = SDL_RWFromFile(relativeExeNewFileName.c_str(), "wb");
		if (!rwo_write) {
			Log(LOG_ERROR) << "Step 10: failed to open exe file for writing." << SDL_GetError();
			return;
		}
		auto wsize = SDL_RWwrite(rwo_write, data, size, 1);
		if (wsize != 1) {
			Log(LOG_ERROR) << "Step 10: failed to write exe file." << SDL_GetError();
			return;
		}
